...
## TEST FIT
log.fit <- glm(diabetes~., family=binomial(link="logit"), data=train) # perfect separation warning
pro.fit <- glm(diabetes~., family=binomial(link="probit"), data=train) # perfect separation warning
#summary(pro.fit)
#summary(log.fit)
